...
```{r}
glimpse(df1)
df1$newdate <- with(df1, as.POSIXct(paste(Date, Time), format="%Y-%m-%d %H:%M:%S"))

plot(numbers ~ newdate, data=dat, type="b", col="blue")

plot2 <- plot(df1$newdate, df1$Global_active_power,
         type="l",
         xlab="",
...
``` {r}
plot3 <- plot(df1$newdate, df1$Sub_metering_1, type="l", col="black",
         xlab="", ylab="Energy sub metering")
    lines(df1$newdate, df1$Sub_metering_2, col="red")
    lines(df1$newdate, df1$Sub_metering_3, col="blue")
    legend("topright",
...
par(mfrow = c(2,2))
plot(df1$newdate, df1$Global_active_power,
         type="l",
         xlab="",
         ylab="Global Active Power")
    # 2
    plot(df1$newdate, df1$Voltage, type="l",
         xlab="datetime", ylab="Voltage")
    # 3
    plot(df1$newdate, df1$Sub_metering_1, type="l", col="black",
         xlab="", ylab="Energy sub metering")
    lines(df1$newdate, df1$Sub_metering_2, col="red")
    lines(df1$newdate, df1$Sub_metering_3, col="blue")
    legend("topright",
           ...
           box.lwd=0)
    # 4
    plot(df1$newdate, df1$Global_reactive_power, type="n",
         xlab="datetime", ylab="Global_reactive_power")
    lines(df1$newdate, df1$Global_reactive_power)
